...
// Organization IDs are prefixed CUIDs (e.g. "org_clx1y2z3...") generated by
// generate_prefixed_cuid() in the database. The Upstash Vector filter below is
// built by raw string interpolation, so any character with meaning in Upstash's
// filter language - a `"` to close the string literal, or GLOB/AND/OR syntax -
// would let a malicious organizationId read another tenant's vectors (GH-103).
// Allowlist to the known ID shape before it ever reaches the filter string.
const SAFE_ORGANIZATION_ID = /^[a-zA-Z0-9_-]+$/;

/**
 * Guards against Upstash Vector filter injection via organizationId.
 * Throws rather than silently dropping the filter, since a caught error here
 * fails closed (both callers already treat a thrown error as "no results").
 */
export function assertSafeOrganizationId(organizationId: string): void {
  ...
  throw new Error(
    `Refusing to build vector filter: organizationId "${organizationId}" contains disallowed characters`,
  );
}
